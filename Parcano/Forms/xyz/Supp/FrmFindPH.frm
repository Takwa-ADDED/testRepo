...
Begin VB.Form Frm_FindPH 
   ...
   Begin SToolBox.SGrid grid1 
      ...
      MaxVisibleRows  =   0
   End
   Begin SToolBox.SCommand Cmd_Quitter 
      Height          =   255
      Left            =   7200
      TabIndex        =   3
      Top             =   0
...
'----------------------------------------------------------------------------------------------------------------------------------
'Subilations Chargement & Partir Form***
'----------------------------------------------------------------------------------------------------------------------------------
...
On Error GoTo Err
    ...
    Select Case StrSource
        ...
        Case "ProgChauffeurs"
            ...
            Call Affiche_ProgChauffeursAvecDetails(Date, Date, 4, ViewSupp)
...
If Not rs.EOF Then
    ...
    While Not rs.EOF
        With grid1
            ...
            If StrSource <> "VehiculePH" Then .CellDetails .Rows, .ColumnIndex("Actif"), rs("Actif")
        End With
        rs.MoveNext
    Wend
    grid1.Redraw = True
...
Public Sub Affiche_FournisseurAchat()
    ...
    End If
End Sub
Public Sub Affiche_ProgChauffeursAvecDetails(ByVal Ddebut As String, ByVal Dfin As String, ByVal Param As Integer, ByVal ViewSupp As String)
    
    Dim Lobj_Find As ProgChauf
    Dim Lrs As Recordset

    Set Lobj_Find = New ProgChauf
    Set Lrs = Lobj_Find.GetRow_ProgramChauffeur(ErrNumber, ErrDescription, ErrSourceDetail, Ddebut, Dfin, Param, ViewSupp, CNB)
    If ErrNumber <> 0 Then
        MsgBox ErrDescription & vbNewLine & ErrSourceDetail, vbQuestion
        ErrNumber = 0
        Exit Sub
...
Public Sub Initgrid1_Vehicule()
    With grid1
        ...
        If StrSource <> "VehiculePH" Then .AddColumn "Actif", "Acif", , , 40
...
Private Sub Cmd_Search_Click()
    ...
    If Obtn_AFaire.Value = vbChecked Then
        grid1.ClearRows
        Call Affiche_ProgChauffeursAvecDetails(Ddebut, Dfin, 4, ViewSupp)
        PixBox_Lister.Visible = False
        Lbl_Msg.Visible = False
        If grid1.Rows = 0 Then
            ...
            Call Affiche_ProgChauffeursAvecDetails(Ddebut, Dfin, 0, ViewSupp)
            PixBox_Lister.Visible = False
            Lbl_Msg.Visible = False
        End If
    End If
